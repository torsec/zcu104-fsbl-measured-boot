...
void XFsbl_ShaDigest(const u8 *In, const u32 Size, u8 *Out, u32 HashLen)
{
	...
	if(XFSBL_HASH_TYPE_SHA3 == HashLen)
	{
        ...
        // Inizializzazione SHA-3
        (void)XSecure_Sha3Initialize(&SecureSha3, &CsuDma);
        XSecure_Sha3Start(&SecureSha3);

        // Variabili per gestire i blocchi
        u32 remaining_data = Size;
        const u8 *current_data_pointer = In;

        // Hash dei dati a blocchi
        while (remaining_data > 0) {
            u32 current_chunk_size = (remaining_data > 1024) ? 1024 : remaining_data;
            
            // Esegui l'hashing del blocco corrente
            XSecure_Sha3Update(&SecureSha3, current_data_pointer, current_chunk_size);
            
            // Aggiorna il puntatore e i dati rimanenti
            current_data_pointer += current_chunk_size;
            remaining_data -= current_chunk_size;
        }

        // Completa il calcolo del digest
        XSecure_Sha3Finish(&SecureSha3, Out);
	}
}
...
void XFsbl_ShaStart(void * Ctx, u32 HashLen)
{
	if(XFSBL_HASH_TYPE_SHA3 == HashLen)
	{
        // XFsbl_Printf(DEBUG_INFO, "sha start-->sha3initialize\r\n");
		(void)XSecure_Sha3Initialize(&SecureSha3, &CsuDma);
        // XFsbl_Printf(DEBUG_INFO, "sha3start\r\n");
		XSecure_Sha3Start(&SecureSha3);
	}
}
