...
/* For MEASURED BOOT, include the measured boot include files */
#ifdef MEASURED_BOOT
...
#ifdef XFSBL_SECURE
/* For MEASURED BOOT, rename the global variable Iv so that it
 * does not conflict with the global variable of the same name
 * in the Xilsecure library
 */
#ifdef MEASURED_BOOT
...
#endif
#endif
/* For MEASURED BOOT, the variable BistreamWordSize is not used */
...
/* For MEASURED BOOT, use the updated global variable name */
#ifdef MEASURED_BOOT
...
#endif
/* For MEASURED BOOT, add the PartitionNum to the PL parameters */
#ifdef MEASURED_BOOT
...
/* For MEASURED BOOT, use the Tpm_Measure_Pl routine rather than AesDecrypt
 * as the Tpm_Measure_Pl routine will also update the PCR
 */
#ifdef MEASURED_BOOT

			Status = (u32)Tpm_Measure_Pl(FsblInstancePtr, PartitionNum, &SecureAes,
			...
			XTime_GetTime(&tCur);
#endif
/* For MEASURED BOOT, use the Tpm_Measure_Pl subroutine to load the PL.
 * It will also handle updating the PCR register. This subroutine
 * handles both the PS_DDR case and the PS DDR-less case.
 */
#ifdef MEASURED_BOOT
			Status = (u32)Tpm_Measure_Pl(FsblInstancePtr, PartitionNum, NULL,
					(u8 *) XFSBL_DESTINATION_PCAP_ADDR,
					(u8 *) LoadAddress);
...
#endif
/* For MEASURED BOOT, measure the authenticated and decrypted partition
 * when the partitions is not a PL partition.
 */
#ifdef MEASURED_BOOT
	if (DestinationDevice != XIH_PH_ATTRB_DEST_DEVICE_PL) {
		Status = Tpm_Measure_Partition(FsblInstancePtr, PartitionNum, LoadAddress);
		if (Status != XFSBL_SUCCESS)
			{
