...
/* For MEASURED BOOT, include the measured boot code */
#ifdef MEASURED_BOOT
...
/* For MEASURED BOOT, rename this global variable so that it doesn't
 * conflict with the global variable of the same name in the
 * Xilsecure library 
 */
#ifdef MEASURED_BOOT
...
/* For MEASURED BOOT, initialize the TPM, run selftests
 * and measure the CSU ROM
 */
#ifdef MEASURED_BOOT
	/* Initialize the TPM, run startup, run selftest, and measure the CSU ROM */
	// Tpm_Init();
	// Tpm_Startup();
	// Tpm_SelfTest();
	Tpm_Measure_Rom();
...
#ifdef XFSBL_SECURE
/* For MEASURED BOOT, use the renamed global variable rather than the named
 * variable used in the standard FSBL.
 */
	XFsbl_MemCpy(Global_FsblIv, ReadBuffer + XIH_BH_IV_OFFSET, XIH_BH_IV_LENGTH); //errore qui
#else
...
/* For MEASURED BOOT, measure the FSBL. Note that since the image
 * header table has been loaded, it and all other modified variables
 * up to thie point become part of the measurement. Therefore, the
 * FSBL data up to here must be deterministic.
 */
#ifdef MEASURED_BOOT
